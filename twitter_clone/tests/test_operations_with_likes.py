...
class TestAddLikeRoute:
    ...
    def test_error_when_requested_with_too_length_api_key(self, client):
        tweet_id = 1
        api_key = "A" * 33
        response = client.post(f"/api/tweets/{tweet_id}/likes", headers={"api-key": api_key})
        assert response.status_code == 422
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
    ...
    def test_error_when_requested_with_not_exist_tweet_id(self, client):
        ...
        assert response.json()["result"] is False
        assert "HTTPException" in response.json()["error_type"]
        assert f"Твита с id {tweet_id} не существует" == response.json()["error_message"]

    async def test_successfully_response_when_requested_with_normal_values(self, client, db_session):
    ...
    async def test_error_when_posted_exist_like(self, client, db_session):
        ...
        response_temp = client.post(f"/api/tweets/{tweet_id}/likes", headers={"api-key": api_key})
        likes_count_before = await Like.get_likes_count(async_session)
        response = client.post(f"/api/tweets/{tweet_id}/likes", headers={"api-key": api_key})
        ...
        assert response.json()["result"] is False
        assert "HTTPException" in response.json()["error_type"]
        assert (f"Запись о добавлении лайка твиту с id {tweet_id} пользователем с id {api_key} уже существует" ==
...
class TestDeleteLikeRoute:
    ...
    def test_error_when_requested_with_too_length_api_key(self, client):
        tweet_id = 1
        api_key = "A" * 33
        response = client.delete(f"/api/tweets/{tweet_id}/likes", headers={"api-key": api_key})
        assert response.status_code == 422
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
        assert "String should have at most 32 characters" in response.json()["error_message"]
    ...
    def test_error_when_requested_with_not_exist_api_key(self, client):
        ...
        assert response.json()["result"] is False
        assert "HTTPException" in response.json()["error_type"]
        assert (f"Запись лайка твита с id {tweet_id} от пользователя с id {api_key} не существует" ==
                response.json()["error_message"])
    ...
    async def test_successfully_response_when_values_is_normal(self, client, db_session):
        async_session = db_session()
        api_key = "test"
        tweet_id = await Tweet.add_tweet(async_session, author_id=api_key, content="It's just for test...")
    ...
    async def test_error_when_like_is_not_exist(self, client, db_session):
        ...
        response_temp = client.delete(f"/api/tweets/{tweet_id}/likes", headers={"api-key": api_key})
        likes_count_before = await Like.get_likes_count(async_session)
        response = client.delete(f"/api/tweets/{tweet_id}/likes", headers={"api-key": api_key})
