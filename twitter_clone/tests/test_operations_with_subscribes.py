...
class TestFollowUserRoute:
    ...
    def test_error_when_requested_with_too_length_api_key(self, client):
        ...
        response = client.post(
            f"/api/users/{user_id}/follow", headers={"api-key": api_key}
        )
        assert response.status_code == 422
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
    ...
    def test_error_when_requested_with_not_exist_user_id(self, client):
        ...
        assert "HTTPException" in response.json()["error_type"]
        assert (
            f"Пользователя с id {user_id} не существует"
            == response.json()["error_message"]
        )
    ...
    async def test_successfully_subscribe_add(self, client, db_session):
        ...
        user = await User.add_user(
            async_session, user_id="test_id_10", name="Testname_10"
        )
        user_id = user.id
        ...
        assert (
            await User.get_subscribes_count(async_session) == prev_subscribes_count + 1
        )
    ...
    async def test_error_when_adding_exist_record(self, client, db_session):
        ...
        async_session = db_session()
        user = await User.add_user(
            async_session, user_id="test_id_11", name="Testname_11"
        )
        user_id = user.id

        client.post(url=f"/api/users/{user_id}/follow", headers={"api-key": api_key})
        response = client.post(
            url=f"/api/users/{user_id}/follow", headers={"api-key": api_key}
        )
        ...
        assert "HTTPException" in response.json()["error_type"]
        assert (
            f"Запись о подписке пользователя с id {api_key} на пользователя с {user_id} уже существует в БД"
            == response.json()["error_message"]
        )
...
class TestUnfollowUserRoute:
    ...
    async def test_successfully_subscribe_delete(self, client, db_session):
        ...
        new_user = await User.add_user(
            async_session, user_id=new_user_id, name="Testname_12"
        )
        await User.follow(
            async_session, follower_user_id="test", following_user_id=new_user_id
        )
        prev_subscribes_count = await User.get_subscribes_count(async_session)

        response = client.delete(
            f"/api/users/{new_user.id}/follow", headers={"api-key": api_key}
        )
    ...
    async def test_error_when_deleting_not_exist_record(self, client, db_session):
        ...
        new_user = await User.add_user(
            async_session, user_id=new_user_id, name="Testname_13"
        )
        await User.follow(
            async_session, follower_user_id="test", following_user_id=new_user_id
        )

        client.delete(f"/api/users/{new_user.id}/follow", headers={"api-key": api_key})
        prev_subscribes_count = await User.get_subscribes_count(async_session)
        response = client.delete(
            f"/api/users/{new_user.id}/follow", headers={"api-key": api_key}
        )
        ...
        assert (
            f"Запись об отписке пользователя с id {api_key} от пользователя с {new_user.id} не существует в БД"
            == response.json()["error_message"]
        )
