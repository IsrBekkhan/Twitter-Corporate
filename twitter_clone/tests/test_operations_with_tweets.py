...
class TestGetTweetsRoute:
    ...
    def test_error_when_requested_with_not_exist_user(self, client):
        ...
        assert f"Пользоватлея с id {api_key} не существует в БД" == response.json()["error_message"]

    async def test_successfully_response_with_following_tweets(self, client, db_session):
        async_session = db_session()
        api_key = "test"
        result_temp_1 = await Tweet.add_tweet(async_session, author_id=api_key, content="Some tweet for test app...")
        result_temp_2 = await Tweet.add_tweet(async_session, author_id=api_key, content="Another tweet for test app..")
...
class TestAddTweetRoute:
    ...
    def test_error_when_requested_without_tweet_data(self, client):
        response = client.post("/api/tweets", headers={"api-key": "test"}, json={"tweet_media_ids": []})
        assert response.status_code == 422
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
        assert "Field required" in response.json()["error_message"]
    ...
    def test_error_when_requested_without_tweet_media_ids(self, client):
        response = client.post("/api/tweets", headers={"api-key": "test"}, json={"tweet_data": "It's test text 1..."})
        assert response.status_code == 422
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
        assert "Field required" in response.json()["error_message"]
    ...
    def test_error_when_posted_too_big_tweet_data(self, client):
        ...
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
        assert "String should have at most 6553 characters" in response.json()["error_message"]
    ...
    def test_error_when_tweet_media_ids_as_list_include_a_not_string_values(self, client):
        tweet_data = ""
    ...
    def test_error_when_tweet_media_ids_list_include_over_10_values(self, client):
        ...
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
        assert ("List should have at most 10 items after validation, not {}".format(len(tweet_media_ids)) in
                response.json()["error_message"])

        assert str(tweet_media_ids) in response.json()["error_message"]

    async def test_successfully_response_if_tweet_media_ids_list_is_empty(self, client, db_session):
        ...
        assert len(tweets_count_before) == len(await Tweet.get_all_tweet_ids(async_session)) - 1
    ...
    def test_error_when_posted_with_not_exist_ids_in_tweet_media_ids(self, client):
        ...
        assert response.json()["result"] is False
        assert "HTTPException" in response.json()["error_type"]
        assert "Получен список id изображений, которых не существуeт в БД:" in response.json()["error_message"]
        assert str(tweet_media_ids) in response.json()["error_message"]

    async def test_successfully_response_if_arguments_is_normal(self, client, db_session):
        api_key = "test"
        ...
        added_tweet: Tweet = await Tweet.get_tweet_by_id(async_session, response.json()["tweet_id"])
...
class TestDeleteTweetRoute:
    ...
    def test_error_when_requested_with_string_type_tweet_id(self, client):
        tweet_id = "ABC"
        api_key = "test"
        response = client.delete(f"/api/tweets/{tweet_id}", headers={"api-key": api_key})
        assert response.status_code == 422
        assert response.json()["result"] is False
        assert "RequestValidationError" in response.json()["error_type"]
    ...
    async def test_error_when_requested_with_not_exist_api_key(self, client, db_session):
        async_session = db_session()
        ...
        assert response.json()["result"] is False
        assert "HTTPException" in response.json()["error_type"]
        assert f"Пользователя с id {api_key} не существует" in response.json()["error_message"]
    ...
    async def test_successfully_response_with_normal_values(self, client, db_session):
        async_session = db_session()
        api_key = "test"
        tweet_id = await Tweet.add_tweet(async_session, author_id=api_key, content="It's just text for text...")
        all_tweets_before = await Tweet.get_all_tweet_ids(async_session)
